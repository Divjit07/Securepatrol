-- Simplified GPS validation: trust admin-entered map coordinates, drop lobby heuristics.
-- Upper floors work when checkpoints use distinct Google Maps pins (not the lobby stack).
-- Run in Supabase SQL Editor after 011_fix_lobby_detection.sql
...
BEGIN
  ...
  -- Optional altitude check only when the phone actually reports it
  IF floor_num > 1
     AND expected_alt IS NOT NULL
     AND NEW.guard_altitude IS NOT NULL
     AND abs(NEW.guard_altitude - expected_alt) > 10 THEN
    NEW.status := 'fail';
    RETURN NEW;
  END IF;
...
UPDATE checkpoints SET radius_metres = 20 WHERE radius_metres < 20;
ALTER TABLE checkpoints ALTER COLUMN radius_metres SET DEFAULT 20;
